...
interface UseTabbyPaymentOptions {
  checkoutType?: CheckoutType;
}

interface UseTabbyPaymentResult {
  isLoading: boolean;
  error?: string;
...
export const useTabbyPayment = (
  options: UseTabbyPaymentOptions = {},
): UseTabbyPaymentResult => {
  ...
  const checkout = useCallback(async () => {
    ...
    try {
      const payload: {
        ...
        checkout_type: checkoutType,
        // Always send dummy notes as empty string for Tabby checkout
        notes: '',
      };
      ...
      const message =
        ...
        'Failed to start Tabby checkout. Please try again.';
      setError(message);
      throw err;
    } finally {
      setIsLoading(false);
...
export default useTabbyPayment;
